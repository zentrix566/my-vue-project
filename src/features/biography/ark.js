...
7. **在世人物规则（最高优先级之一）**：若用户提供的检索资料中没有任何死亡信息——无"逝世日期"、无去世/病逝/离世/死因等记载——说明该人物可能仍在世。此时**绝对禁止编造死亡年份、死因或"终年"**：首行生卒年写"出生年—至今"（如"许家印（1958年10月9日—至今），中国恒大集团原董事局主席"）；事迹行只列到最近可考的事件；最后一行写成"至今在世"（不加句号）。只有当资料明确记载了死亡（逝世日期、死因）时，才按规则 4 写死亡行。
   **古代人物例外（务必注意）**：若资料显示此人出生于公元前或 1900 年以前（如"前175年""1037年"），或摘要开头括号里给了生卒年范围、或正文描述了其死亡/追谥/身后事，则无论系统是否提示"疑似在世"，一律按已故人物处理——这类人物不可能仍在世，缺"逝世日期"字段只是百科卡片未填，不是在世证据。例如资料写"卓文君（前175年～前121年）"就必须按前121年去世整理，绝不能写成"前175年—至今""至今在世"。`
...
export async function fetchBiography(name) {
  ...
  if (alive) {
    user += `\n\n【重要】系统判断此人可能仍在世（百科未登记逝世日期）。请严格按规则 7：不得编造死亡年份、死因或"终年"，首行写"出生年—至今"，末行写"至今在世"。但请先核对资料：若此人出生于公元前或 1900 年以前，或摘要开头括号里已写明卒年、或正文记载了其死亡/追谥/身后事，则属古代已故人物，按正常死亡行整理，不受本在世提示约束。`
  }
  ...
  // 若末行漏写"终年"，或写成"终年不详"但首行生卒年可算出享年，则自动计算补全
  if (lines.length >= 2) {
    const header = lines[0]
    ...
    if (lifeMatch) {
      ...
      } else if (age !== null && /终年不详/.test(deathLine)) {
        // 模型写了"终年不详"但享年其实可由生卒年推算，替换为准确岁数
        lines[lines.length - 1] = deathLine.replace(/终年不详/, `终年${age}岁`)
      }
    }
  }
  ...
  // 仅对近现代（生年 ≥ 1900）人物生效：古代人物若被误报在世，模型通常已正确写出卒年，
  // 这里不能再强行抹掉死亡行。
  if (alive && lines.length) {
    const header = lines[0]
    const birthM = header.match(/（\s*(约?前?\d+)年/)
    let birthYear = null
    if (birthM) {
      const bc = /前/.test(birthM[1])
      const n = parseInt(birthM[1].replace(/约|前/g, ''), 10)
      if (!Number.isNaN(n)) birthYear = bc ? -n : n
    }
    if (birthYear == null || birthYear >= 1900) {
      ...
      lines[0] = lines[0].replace(
        /(\uff08\s*约?前?\d+年(?:\d+月\d+日)?\s*[\u2014\u2013\u002d\uff0d\u007e\uff5e]\s*)约?前?\d+年(?:\d+月\d+日)?(\s*\uff09)/,
        '$1至今$2'
      )
    }
  }
